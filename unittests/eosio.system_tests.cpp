...
BOOST_FIXTURE_TEST_CASE( buysell, eosio_system_tester ) try {
   ...
   transfer( "actx", "alice1111111", core_from_string("1000.0000"), "actx" );
   BOOST_REQUIRE_EQUAL( success(), stake( "actx", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000") ) );

   auto total = get_total_stake( "alice1111111" );
   auto init_bytes =  total["ram_bytes"].as_uint64();

   const asset initial_ram_balance = get_balance(N(actx.ram));
   const asset initial_ramfee_balance = get_balance(N(actx.ramfee));
   ...
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_from_string("199.0000"), get_balance(N(actx.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_from_string("1.0000"), get_balance(N(actx.ramfee)) );
   ...
   transfer( "actx", "alice1111111", core_from_string("100000000.0000"), "actx" );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, eosio_system_tester ) try {
   ...
   const auto init_eosio_stake_balance = get_balance( N(actx.stake) );
   ...
   BOOST_REQUIRE_EQUAL( init_eosio_stake_balance + core_from_string("300.0000"), get_balance( N(actx.stake) ) );
   ...
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_from_string("1000.0000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( init_eosio_stake_balance, get_balance( N(actx.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, eosio_system_tester ) try {
   ...
   issue( "actx", core_from_string("1000.0000"), config::system_account_name );
   issue( "actx.stake", core_from_string("1000.0000"), config::system_account_name );
   BOOST_REQUIRE_EQUAL( core_from_string("0.0000"), get_balance( "alice1111111" ) );

   //eosio stakes for alice with transfer flag

   transfer( "actx", "bob111111111", core_from_string("1000.0000"), "actx" );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "actx", "alice1111111", core_from_string("2000.0000"), core_from_string("1000.0000") ) );
...
BOOST_FIXTURE_TEST_CASE(producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      const int64_t  initial_savings           = get_balance(N(actx.saving)).get_amount();
      ...
      const int64_t  savings           = get_balance(N(actx.saving)).get_amount();
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...

   {
      ...
      const uint32_t initial_tot_unpaid_blocks = initial_global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    initial_supply            = get_token_supply();
      const asset    initial_bpay_balance      = get_balance(N(actx.bpay));
      const asset    initial_vpay_balance      = get_balance(N(actx.vpay));
      ...
      const uint32_t tot_unpaid_blocks = global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    supply            = get_token_supply();
      const asset    bpay_balance      = get_balance(N(actx.bpay));
      const asset    vpay_balance      = get_balance(N(actx.vpay));
...

   {
      ...
      BOOST_REQUIRE_EQUAL( error("missing authority of actx"),
...
BOOST_FIXTURE_TEST_CASE(producers_upgrade_system_contract, eosio_system_tester) try {
   ...
   string actx_system_wast2 = actx_system_wast;
   string msg = "producer votes must be unique and sorted";
   auto pos = actx_system_wast2.find(msg);
   BOOST_REQUIRE( pos != std::string::npos );
   msg[0] = 'P';
   actx_system_wast2.replace( pos, msg.size(), msg );

   transaction trx;
   {
      auto code = wast_to_wasm( actx_system_wast2 );
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, eosio_system_tester ) try {
   ...
   transfer( "actx", "alice1111111", core_from_string("600000000.0000"), "actx" );
...
BOOST_FIXTURE_TEST_CASE( buyname, eosio_system_tester ) try {
   ...
   transfer( "actx", "nofail", core_from_string( "1000.0000" ) );
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, eosio_system_tester ) try {
   ...
   {
      const asset initial_names_balance = get_balance(N(actx.names));
      BOOST_REQUIRE_EQUAL( success(),
      ...
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_from_string("0.1001"), get_balance(N(actx.names)) );
   }
