/**
 *  @file
 *  @copyright defined in Achain2.0/LICENSE.txt
 */
...
asset get_currency_balance(const TESTER& chain, account_name account) {
   return chain.get_currency_balance(N(actx.token), symbol(SY(4,CUR)), account);
}

const std::string actx_token = name(N(actx.token)).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.create_account(N(actx.token));
   chain.produce_blocks(10);

   chain.set_code(N(actx.token), contracts::actx_token_wasm());
   chain.set_abi(N(actx.token), contracts::actx_token_abi().data());
   ...
   chain.push_action(config::system_account_name, linkauth::get_name(), tester_account, fc::mutable_variant_object()
           ...
           ("requirement", "first"));
   chain.produce_blocks();
   chain.push_action(N(actx.token), N(create), N(actx.token), mutable_variant_object()
           ("issuer", actx_token)
           ("maximum_supply", "9000000.0000 CUR")
   );


   chain.push_action(N(actx.token), name("issue"), N(actx.token), fc::mutable_variant_object()
           ("to",       actx_token)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action(N(actx.token), name("transfer"), N(actx.token), fc::mutable_variant_object()
       ("from", actx_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, N(actx.token));
   ...
   trace = chain.push_action(N(actx.token), name("transfer"), N(tester), fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, N(actx.token));
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester));
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   trace = chain.push_action(N(actx.token), name("transfer"), N(actx.token), fc::mutable_variant_object()
       ("from", "actx.token")
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>(N(actx.token));
   const auto abi = acnt.get_abi();
   chain::abi_serializer abis(abi, chain.abi_serializer_max_time);
   const auto a = chain.control->db().get<account_object,by_name>(N(actx.token)).get_abi();
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action(N(actx.token), name("transfer"), vector<permission_level>{{N(tester), N(first)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(actx.token), name("transfer"), vector<permission_level>{{N(tester), N(second)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(actx.token), name("transfer"), vector<permission_level>{{N(tester), config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
