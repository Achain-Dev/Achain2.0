...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   c.set_code( N(act.token), contracts::act_token_wasm() );
   c.set_abi( N(act.token), contracts::act_token_abi().data() );
   c.produce_blocks(10);
   ...
   cr = c.push_action( N(act.token), N(transfer), config::system_account_name, mutable_variant_object()
              ("from",     "act")
              ("to",       "dan" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
