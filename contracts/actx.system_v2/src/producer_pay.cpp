...
namespace actxsystem {
   ...
   void system_contract::claimrewards( const name owner ) {
      ...
      actx_assert( prod.active(), "producer does not have an active key" );
      ...
      actx_assert( ct - prod.last_claim_time > microseconds(useconds_per_day), "already claimed rewards within past day" );

      const asset token_supply   = actx::token::get_supply(token_account, core_symbol().code() );
      ...
      if( usecs_since_last_fill > 0 && _gstate.last_pervote_bucket_fill > time_point() ) {
         ...
         INLINE_ACTION_SENDER(actx::token, issue)(
