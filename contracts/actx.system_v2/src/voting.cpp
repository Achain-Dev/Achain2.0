...
namespace actxsystem {
   ...
   using actx::print;
   using actx::singleton;
   using actx::transaction;
    ...
    *
    */
   void system_contract::regproducer( const name producer, const actx::public_key& producer_key, const std::string& url, uint16_t location ) {
      actx_assert( url.size() < 512, "url too long" );
      actx_assert( producer_key != actx::public_key(), "public key should not be the default value" );
   ...
   void system_contract::update_elected_producers( block_timestamp block_time ) {
      ...
      std::vector< std::pair<actx::producer_key,uint16_t> > top_producers;
      top_producers.reserve(21);

      for ( auto it = idx.cbegin(); it != idx.cend() && top_producers.size() < 21 && 0 < it->total_votes && it->active(); ++it ) {
         top_producers.emplace_back( std::pair<actx::producer_key,uint16_t>({{it->owner, it->producer_key}, it->location}) );
      }
      ...
      std::vector<actx::producer_key> producers;
   ...
   void system_contract::update_votes( const name voter_name, const name proxy, const std::vector<name>& producers, bool voting ) {
      //validate input
      if ( proxy ) {
         actx_assert( producers.size() == 0, "cannot vote for producers and proxy at same time" );
         actx_assert( voter_name != proxy, "cannot proxy to self" );
      } else {
         actx_assert( producers.size() <= 30, "attempt to vote for too many producers" );
         for( size_t i = 1; i < producers.size(); ++i ) {
            actx_assert( producers[i-1] < producers[i], "producer votes must be unique and sorted" );
         }
      }

      auto voter = _voters.find( voter_name.value );
      actx_assert( voter != _voters.end(), "user must stake before they can vote" ); /// staking creates voter object
      actx_assert( !proxy || !voter->is_proxy, "account registered as a proxy is not allowed to use a proxy" );
      ...
      if ( voter->last_vote_weight > 0 ) {
         if( voter->proxy ) {
            ...
            actx_assert( old_proxy != _voters.end(), "old proxy not found" ); //data corruption
      ...
      if( proxy ) {
         ...
         actx_assert( new_proxy != _voters.end(), "invalid proxy specified" ); //if ( !voting ) { data corruption } else { wrong vote }
         actx_assert( !voting || new_proxy->is_proxy, "proxy not found" );
      ...
      for( const auto& pd : producer_deltas ) {
         ...
         if( pitr != _producers.end() ) {
            actx_assert( !voting || pitr->active() || !pd.second.second /* not from new set */, "producer is not currently registered" );
            ...
            _producers.modify( pitr, same_payer, [&]( auto& p ) {
               ...
               //actx_assert( p.total_votes >= 0, "something bad happened" );
            });
            ...
            actx_assert( !pd.second.second /* not from new set */, "producer is not registered" ); //data corruption
         }
      }
   ...
   void system_contract::regproxy( const name proxy, bool isproxy ) {
      ...
      if ( pitr != _voters.end() ) {
         actx_assert( isproxy != pitr->is_proxy, "action has no effect" );
         actx_assert( !isproxy || !pitr->proxy, "account that uses a proxy is not allowed to become a proxy" );
   ...
   void system_contract::propagate_weight_change( const voter_info& voter ) {
      actx_assert( !voter.proxy || !voter.is_proxy, "account registered as a proxy is not allowed to use a proxy" );
