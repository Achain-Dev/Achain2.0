...
#include <actxlib/actx.hpp>
#include <actxlib/print.hpp>
#include <actxlib/datastream.hpp>
#include <actxlib/serialize.hpp>
#include <actxlib/multi_index.hpp>
#include <actxlib/privileged.h>
#include <actxlib/transaction.hpp>
...
namespace actxsystem {
   ...
   struct [[actx::table, actx::contract("actx.system")]] user_resources {
      name          owner;
      asset         net_weight;
      asset         cpu_weight;
      int64_t       ram_bytes = 0;
    ...
    */
   struct [[actx::table, actx::contract("actx.system")]] delegated_bandwidth {
      name          from;
      name          to;
      asset         net_weight;
      asset         cpu_weight;
   ...
   struct [[actx::table, actx::contract("actx.system")]] refund_request {
      name            owner;
      time_point_sec  request_time;
      actx::asset    net_amount;
      actx::asset    cpu_amount;
    ...
    */
   typedef actx::multi_index< "userres"_n, user_resources >      user_resources_table;
   typedef actx::multi_index< "delband"_n, delegated_bandwidth > del_bandwidth_table;
   typedef actx::multi_index< "refunds"_n, refund_request >      refunds_table;
   ...
   void system_contract::buyram( name payer, name receiver, asset quant )
   {
      ...
      actx_assert( quant.symbol == core_symbol(), "must buy ram with core token" );
      actx_assert( quant.amount > 0, "must purchase a positive amount" );
      ...
      actx_assert( bytes_out > 0, "must reserve a positive amount" );
  ...
  /**
   ...
   void system_contract::sellram( name account, int64_t bytes ) {
      ...
      actx_assert( bytes > 0, "cannot sell negative byte" );

      user_resources_table  userres( _self, account.value );
      auto res_itr = userres.find( account.value );
      actx_assert( res_itr != userres.end(), "no resource row" );
      actx_assert( res_itr->ram_bytes >= bytes, "insufficient quota" );
      ...
      actx_assert( tokens_out.amount > 1, "token amount received from selling ram is too low" );
      ...
      actx_assert( _gstate.total_ram_stake >= 0, "error, attempt to unstake more tokens than previously staked" );
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      actx_assert( max_claimable - claimable <= stake, "b1 can only claim their tokens over 10 years" );
   }
   ...
   {
      require_auth( from );
      actx_assert( stake_net_delta.amount != 0 || stake_cpu_delta.amount != 0, "should stake non-zero amount" );
      actx_assert( std::abs( (stake_net_delta + stake_cpu_delta).amount )
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         actx_assert( 0 <= itr->net_weight.amount, "insufficient staked net bandwidth" );
         actx_assert( 0 <= itr->cpu_weight.amount, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         actx_assert( 0 <= tot_itr->net_weight.amount, "insufficient staked total net bandwidth" );
         actx_assert( 0 <= tot_itr->cpu_weight.amount, "insufficient staked total cpu bandwidth" );
      ...
      if ( stake_account != source_stake_from ) { //for actx both transfer and refund make no sense
         ...
         if( is_delegating_to_self || is_undelegating ) {
            if ( req != refunds_tbl.end() ) { //need to update refund
               ...
               actx_assert( 0 <= req->net_amount.amount, "negative net refund amount" ); //should never happen
               actx_assert( 0 <= req->cpu_amount.amount, "negative cpu refund amount" ); //should never happen
      ...
      // update voting power
      {
         ...
         actx_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == "b1"_n ) {
                                     ...
                                     asset stake_cpu_quantity, bool transfer )
   {
      ...
      actx_assert( stake_cpu_quantity >= zero_asset, "must stake a positive amount" );
      actx_assert( stake_net_quantity >= zero_asset, "must stake a positive amount" );
      actx_assert( stake_net_quantity.amount + stake_cpu_quantity.amount > 0, "must stake a positive amount" );
      actx_assert( !transfer || from != receiver, "cannot use transfer flag if delegating to self" );
   ...
   {
      asset zero_asset( 0, core_symbol() );
      actx_assert( unstake_cpu_quantity >= zero_asset, "must unstake a positive amount" );
      actx_assert( unstake_net_quantity >= zero_asset, "must unstake a positive amount" );
      actx_assert( unstake_cpu_quantity.amount + unstake_net_quantity.amount > 0, "must unstake a positive amount" );
      actx_assert( _gstate.total_activated_stake >= min_activated_stake,
   ...
   void system_contract::refund( const name owner ) {
      ...
      actx_assert( req != refunds_tbl.end(), "refund request not found" );
      actx_assert( req->request_time + seconds(refund_delay_sec) <= current_time_point(),
