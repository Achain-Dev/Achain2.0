#include <actx.system/actx.system.hpp>
#include <actxlib/dispatcher.hpp>
#include <actxlib/crypto.h>
...
namespace actxsystem {
    ...
    _rammarket(_self, _self.value)
   {
      ...
      _gstate2 = _global2.exists() ? _global2.get() : actx_global_state2{};
      _gstate3 = _global3.exists() ? _global3.get() : actx_global_state3{};
   }

   actx_global_state system_contract::get_default_parameters() {
      actx_global_state dp;
   ...
   void system_contract::setram( uint64_t max_ram_size ) {
      ...
      actx_assert( _gstate.max_ram_size < max_ram_size, "ram may only be increased" ); /// decreasing ram might result market maker issues
      actx_assert( max_ram_size < 1024ll*1024*1024*1024*1024, "ram size is unrealistic" );
      actx_assert( max_ram_size > _gstate.total_ram_bytes_reserved, "attempt to set max below reserved" );
   ...
   void system_contract::setparams( const actx::blockchain_parameters& params ) {
      require_auth( _self );
      (actx::blockchain_parameters&)(_gstate) = params;
      actx_assert( 3 <= _gstate.max_authority_depth, "max_authority_depth should be at least 3" );
   ...
   void system_contract::setalimits( name account, int64_t ram, int64_t net, int64_t cpu ) {
      ...
      actx_assert( ritr == userres.end(), "only supports unlimited accounts" );
      ...
      if( vitr != _voters.end() ) {
         ...
         actx_assert( !(ram_managed || net_managed || cpu_managed), "cannot use setalimits on an account with managed resources" );
      }
   ...
   void system_contract::setacctram( name account, std::optional<int64_t> ram_bytes ) {
      ...
      if( !ram_bytes ) {
         auto vitr = _voters.find( account.value );
         actx_assert( vitr != _voters.end() && has_field( vitr->flags1, voter_info::flags1_fields::ram_managed ),
         ...
         actx_assert( *ram_bytes >= 0, "not allowed to set RAM limit to unlimited" );
   ...
   void system_contract::setacctnet( name account, std::optional<int64_t> net_weight ) {
      ...
      if( !net_weight ) {
         auto vitr = _voters.find( account.value );
         actx_assert( vitr != _voters.end() && has_field( vitr->flags1, voter_info::flags1_fields::net_managed ),
         ...
         actx_assert( *net_weight >= -1, "invalid value for net_weight" );
   ...
   void system_contract::setacctcpu( name account, std::optional<int64_t> cpu_weight ) {
      ...
      if( !cpu_weight ) {
         auto vitr = _voters.find( account.value );
         actx_assert( vitr != _voters.end() && has_field( vitr->flags1, voter_info::flags1_fields::cpu_managed ),
         ...
         actx_assert( *cpu_weight >= -1, "invalid value for cpu_weight" );
   ...
   void system_contract::rmvproducer( name producer ) {
      ...
      actx_assert( prod != _producers.end(), "producer not found" );
   ...
   void system_contract::updtrevision( uint8_t revision ) {
      require_auth( _self );
      actx_assert( _gstate2.revision < 255, "can not increment revision" ); // prevent wrap around
      actx_assert( revision == _gstate2.revision + 1, "can only increment revision by one" );
      actx_assert( revision <= 1, // set upper bound to greatest revision supported in the code
   ...
   void system_contract::bidname( name bidder, name newname, asset bid ) {
      require_auth( bidder );
      actx_assert( newname.suffix() == newname, "you can only bid on top-level suffix" );

      actx_assert( (bool)newname, "the empty name is not a valid account name to bid on" );
      actx_assert( (newname.value & 0xFull) == 0, "13 character names are not valid account names to bid on" );
      actx_assert( (newname.value & 0x1F0ull) == 0, "accounts with 12 character names and no dots can be created without bidding required" );
      actx_assert( !is_account( newname ), "account already exists" );
      actx_assert( bid.symbol == core_symbol(), "asset must be system token" );
      actx_assert( bid.amount > 0, "insufficient bid" );

      INLINE_ACTION_SENDER(actx::token, transfer)(
      ...
      if( current == bids.end() ) {
         ...
         actx_assert( current->high_bid > 0, "this auction has already closed" );
         actx_assert( bid.amount - current->high_bid > (current->high_bid / 10), "must increase bid by 10%" );
         actx_assert( current->high_bidder != bidder, "account is already highest bidder" );
   ...
   void system_contract::bidrefund( name bidder, name newname ) {
      ...
      actx_assert( it != refunds_table.end(), "refund not found" );
      INLINE_ACTION_SENDER(actx::token, transfer)(
      ...
      if( creator != _self ) {
         ...
         if( has_dot ) { // or is less than 12 characters
            ...
            if( suffix == newact ) {
               ...
               actx_assert( current != bids.end(), "no active bid for name" );
               actx_assert( current->high_bidder == creator, "only highest bidder can claim" );
               actx_assert( current->high_bid < 0, "auction for name is not closed yet" );
               bids.erase( current );
            } else {
               actx_assert( creator == suffix, "only suffix may create this account" );
            }
         }
      }
   ...
   void native::setabi( name acnt, const std::vector<char>& abi ) {
      actx::multi_index< "abihash"_n, abi_hash >  table(_self, _self.value);
   ...
   void system_contract::init( unsigned_int version, symbol core ) {
      require_auth( _self );
      actx_assert( version.value == 0, "unsupported version for init action" );

      auto itr = _rammarket.find(ramcore_symbol.raw());
      actx_assert( itr == _rammarket.end(), "system contract has already been initialized" );

      auto system_token_supply   = actx::token::get_supply(token_account, core.code() );
      actx_assert( system_token_supply.symbol == core, "specified core symbol does not exist (precision mismatch)" );

      actx_assert( system_token_supply.amount > 0, "system token supply must be greater than 0" );
...
actx_DISPATCH( actxsystem::system_contract,
     // native.hpp (newaccount definition is actually in actx.system.cpp)
