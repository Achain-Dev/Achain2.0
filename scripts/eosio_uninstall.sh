...
function usage() {
  ...
  -i DIR      Directory where Achain is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the Achain data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
   ...
   while true; do
      ...
      case $PROCEED in
         ...
         0 | true | [Yy]* )
            ...
            break;;
         1 | false | [Nn]* ) echo " - Cancelled Achain Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi
