## Overview
This guide provides instructions on how to connect to specifc Achain blockchain when using `cleos`. `Cleos` can connect to a specific node by using the `--url` optional argument, followed by the http address and port number.
...
[[info | Note]]
| `Cleos` is bundled with the Achain software.Install Achain will install the `cleos` and `keosd` command line tools.  
* You have access to an Achain blockchain and the http afddress and port number of a `nodeos` instance. 
