...
namespace eosio { namespace chain {
   ...
   void abi_serializer::set_abi(const abi_def& abi, const fc::microseconds& max_serialization_time) {
      ...
      for( const auto& td : abi.types ) {
         ...
         EOS_ASSERT(!_is_type(td.new_type_name, ctx), duplicate_abi_type_def_exception, "type already exists", ("new_type_name",impl::limit_size(td.new_type_name)));
   ...
   bool abi_serializer::is_array(const std::string_view& type)const {
      return ends_with(type, "[]");
   }

   bool abi_serializer::is_optional(const std::string_view& type)const {
      return ends_with(type, "?");
   }
   ...
   void abi_serializer::validate( impl::abi_traverse_context& ctx )const {
      for( const auto& t : typedefs ) { try {
         ...
         while( itr != typedefs.end() ) {
            ctx.check_deadline();
            EOS_ASSERT( find(types_seen.begin(), types_seen.end(), itr->second) == types_seen.end(), abi_circular_def_exception, "Circular reference in type ${type}", ("type",impl::limit_size(t.first)) );
      ...
      for( const auto& s : structs ) { try {
         if( s.second.base != type_name() ) {
            ...
            while( current->base != type_name() ) {
               ...
               EOS_ASSERT( find(types_seen.begin(), types_seen.end(), base.name) == types_seen.end(), abi_circular_def_exception,"Circular reference in struct ${type}", ("type",impl::limit_size(s.second.name)) );
         ...
         for( const auto& field : s.second.fields ) { try {
            ctx.check_deadline();
            EOS_ASSERT(_is_type(_remove_bin_extension(field.type), ctx), invalid_type_inside_abi, "${type}", ("type",impl::limit_size(field.type)) );
   ...
   namespace impl {
      ...
      void output_name( std::ostream& s, const std::string_view& str, bool shorten, size_t max_length = 64 ) {
