...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = N(act.token) )
{
...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   set_code( N(eosio.token), contracts::act_token_wasm() );
   set_abi( N(eosio.token), contracts::act_token_abi().data() );
   produce_blocks(1);
   ...
   eosio::chain_apis::read_only::get_table_by_scope_params param{N(act.token), N(accounts), "inita", "", 10};
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   create_accounts({ N(act.token), N(act.ram), N(act.ramfee), N(act.stake),
      N(eosio.bpay), N(act.vpay), N(act.saving), N(act.names) });
